...
const websocket_event_1 = require("./websocket_event");
/**
...
class Websocket {
    ...
    constructor(url, protocols, options) {
         ...
         */
        this.handleOpenEvent = (event) => this.handleEvent(websocket_event_1.WebsocketEvent.open, event);
        /**
         * Handles the 'error' event of the browser-native websocket.
         * @param event to handle.
         */
        this.handleErrorEvent = (event) => this.handleEvent(websocket_event_1.WebsocketEvent.error, event);
        /**
         * Handles the 'close' event of the browser-native websocket.
         * @param event to handle.
         */
        this.handleCloseEvent = (event) => this.handleEvent(websocket_event_1.WebsocketEvent.close, event);
        /**
         * Handles the 'message' event of the browser-native websocket.
         * @param event to handle.
         */
        this.handleMessageEvent = (event) => this.handleEvent(websocket_event_1.WebsocketEvent.message, event);
        this._url = url;
            ...
            switch (type) {
                ...
                case websocket_event_1.WebsocketEvent.open:
                    if (this.backoff !== undefined && this._lastConnection !== undefined) {
                        ...
                        const event = new CustomEvent(websocket_event_1.WebsocketEvent.reconnect, {
                            detail,
                        });
                        yield this.dispatchEvent(websocket_event_1.WebsocketEvent.reconnect, event);
                        this.backoff.reset();
                    }
    ...
    scheduleConnectionRetryIfNeeded() {
        ...
        const handleRetryEvent = (detail) => {
            const event = new CustomEvent(websocket_event_1.WebsocketEvent.retry, { detail });
            this.handleEvent(websocket_event_1.WebsocketEvent.retry, event);
        };
