...
describe("Testsuite for ConstantBackoff", () => {
    test("Initialization should throw on negative backoff", () => {
        expect(() => new src_1.ConstantBackoff(-1001)).toThrow();
        expect(() => new src_1.ConstantBackoff(-42)).toThrow();
        expect(() => new src_1.ConstantBackoff(-1)).toThrow();
    });
    test("Initialization should not throw on zero backoff", () => {
        expect(() => new src_1.ConstantBackoff(0)).not.toThrow();
    });
    test("Initialization should not throw on positive backoff", () => {
        expect(() => new src_1.ConstantBackoff(1)).not.toThrow();
        expect(() => new src_1.ConstantBackoff(42)).not.toThrow();
        expect(() => new src_1.ConstantBackoff(1001)).not.toThrow();
    });
    test("Backoff should be equal to the given backoff", () => {
        expect(new src_1.ConstantBackoff(1).current).toBe(1);
        expect(new src_1.ConstantBackoff(42).current).toBe(42);
        expect(new src_1.ConstantBackoff(1001).current).toBe(1001);
    });
    ...
    test("Retries should be zero after initialization", () => {
        expect(new src_1.ConstantBackoff(42).retries).toBe(0);
    });
