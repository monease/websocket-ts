...
describe("Testsuite for WebSocketBuilder", () => {
    ...
    test("WebsocketBuilder should set url", () => {
        const builder = new src_1.WebsocketBuilder(url);
    ...
    test("WebsocketBuilder should set protocols", () => {
        ...
        const builder = new src_1.WebsocketBuilder(url).withProtocols(protocols);
    ...
    test("WebsocketBuilder should set max-retries", () => {
        const maxRetries = 42;
        const builder = new src_1.WebsocketBuilder(url).withMaxRetries(maxRetries);
    ...
    test("WebsocketBuilder should set instant-reconnect", () => {
        ...
        const builder = new src_1.WebsocketBuilder(url).withInstantReconnect(instantReconnect);
    ...
    test("WebsocketBuilder should set backoff", () => {
        const backoff = new src_1.ConstantBackoff(42);
        const builder = new src_1.WebsocketBuilder(url).withBackoff(backoff);
    ...
    test("WebsocketBuilder should set backoff for subsequent calls", () => {
        const backoff1 = new src_1.ConstantBackoff(42);
        const backoff2 = new src_1.ConstantBackoff(1337);
        const builder = new src_1.WebsocketBuilder(url)
    ...
    test("WebsocketBuilder should set buffer", () => {
        ...
        const builder = new src_1.WebsocketBuilder(url).withBuffer(buffer);
    ...
    test("WebsocketBuilder should set buffer for subsequent calls", () => {
        const buffer1 = new src_1.ArrayQueue();
        const buffer2 = new src_1.ArrayQueue();
        const builder = new src_1.WebsocketBuilder(url)
            .withBuffer(buffer1)
            .withBuffer(buffer2);
    ...
    test("WebsocketBuilder should set 'open'-listener", () => {
        ...
        const builder = new src_1.WebsocketBuilder(url).onOpen(listener);
    ...
    test("WebsocketBuilder should set 'open'-listener with options", () => {
        ...
        const builder = new src_1.WebsocketBuilder(url).onOpen(listener, options);
        expect(builder["_options"]["listeners"].open).toStrictEqual([{ listener, options }]);
        const ws = builder.build();
        expect(ws["_options"]["listeners"].open).toStrictEqual([{ listener, options }]);
    });
    ...
    test("WebsocketBuilder should set 'close'-listener", () => {
        const listener = jest.fn();
        const builder = new src_1.WebsocketBuilder(url).onClose(listener);
    ...
    test("WebsocketBuilder should set 'close'-listener with options", () => {
        const listener = jest.fn();
        const options = { once: true };
        const builder = new src_1.WebsocketBuilder(url).onClose(listener, options);
        expect(builder["_options"]["listeners"].close).toStrictEqual([
            {
                listener,
                options,
        ...
        const ws = builder.build();
        expect(ws["_options"]["listeners"].close).toStrictEqual([{ listener, options }]);
    });
    ...
    test("WebsocketBuilder should set 'error'-listener", () => {
        const listener = jest.fn();
        const builder = new src_1.WebsocketBuilder(url).onError(listener);
    ...
    test("WebsocketBuilder should set 'error'-listener with options", () => {
        const listener = jest.fn();
        const options = { once: true };
        const builder = new src_1.WebsocketBuilder(url).onError(listener, options);
        expect(builder["_options"]["listeners"].error).toStrictEqual([
            {
                listener,
                options,
        ...
        const ws = builder.build();
        expect(ws["_options"]["listeners"].error).toStrictEqual([{ listener, options }]);
    });
    ...
    test("WebsocketBuilder should set 'message'-listener", () => {
        const listener = jest.fn();
        const builder = new src_1.WebsocketBuilder(url).onMessage(listener);
    ...
    test("WebsocketBuilder should set 'message'-listener with options", () => {
        const listener = jest.fn();
        const options = { once: true };
        const builder = new src_1.WebsocketBuilder(url).onMessage(listener, options);
        expect(builder["_options"]["listeners"].message).toStrictEqual([
            {
                listener,
                options,
    ...
    test("WebsocketBuilder should set 'retry'-listener", () => {
        const listener = jest.fn();
        const builder = new src_1.WebsocketBuilder(url).onRetry(listener);
    ...
    test("WebsocketBuilder should set 'retry'-listener with options", () => {
        const listener = jest.fn();
        const options = { once: true };
        const builder = new src_1.WebsocketBuilder(url).onRetry(listener, options);
        expect(builder["_options"]["listeners"].retry).toStrictEqual([
            {
                listener,
                options,
        ...
        const ws = builder.build();
        expect(ws["_options"]["listeners"].retry).toStrictEqual([{ listener, options }]);
    });
    ...
    test("WebsocketBuilder should set 'reconnect'-listener", () => {
        const listener = jest.fn();
        const builder = new src_1.WebsocketBuilder(url).onReconnect(listener);
    ...
    test("WebsocketBuilder should set 'reconnect'-listener with options", () => {
        const listener = jest.fn();
        const options = { once: true };
        const builder = new src_1.WebsocketBuilder(url).onReconnect(listener, options);
        expect(builder["_options"]["listeners"].reconnect).toStrictEqual([
            {
                listener,
                options,
    ...
    test("WebsocketBuilder should return a Websocket instance", () => {
        const builder = new src_1.WebsocketBuilder(url);
        const ws = builder.build();
        expect(ws).toBeInstanceOf(src_1.Websocket);
    });
...
//# sourceMappingURL=websocket_builder.test.js.map